...
<div>
    <div id="map" class="large-header">
      ...
      <div v-if="obj2" class="tripstop-wrapper" v-for="item in obj2" v-bind:key="item.name">
    ...
    <h1 class="card-subheadline">{{item.subname}}</h1>
      ...
      <div v-else class="tripstop-wrapper" v-for="item in obj" v-bind:key="item.name">
   <span class="waypoint"></span>
        <div class="card">
          <div class="delete-card" @click="removeCard"></div>
          <div class="tripstop-card-wrapper">
 ...
 var popup = new mapboxgl.Popup({ offset: 25 }).setHTML(
   ...
   popup.on('open', function (e) {
    ...
    deleteTrip1: async function(){
      ...
      this.$router.go({name: 'PlannedTrips'})
    },
